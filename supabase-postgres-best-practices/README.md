...
A skill that gives AI agents a comprehensive set of Postgres performance optimization references for Supabase projects. When installed, the agent automatically applies best practices for indexing, connection management, RLS policies, schema design, and more — catching common mistakes and suggesting high-impact improvements during code generation and review.

This skill follows the [Agent Skills Open Standard](https://agentskills.io/).

> [!NOTE]
> `AGENTS.md` is auto-generated by the build system from individual reference files in `references/`. Do not edit it directly — edit the source references and run `npm run build`.
