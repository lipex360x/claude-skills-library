...
- Include all necessary context inline (specs, tokens, templates)
...
Choose based on the user's need for visibility. If the skill produces visual output the user wants to watch appear progressively, use foreground.

## Two-phase build (setup → agents)

When agents depend on infrastructure being ready (files created, manifests populated, UI state set), split the work into two sequential phases:

**Phase A — Setup** (single response, all in parallel):
- Create project/files/scaffolding
- Navigate the user to the right view
- Populate manifests and skeleton content
...
**Why sequential?** If agents run before setup completes, they race against infrastructure creation. This was discovered in production when agents tried to write to a manifest that didn't exist yet because `create_artboards` hadn't finished. The agents succeeded in writing files, but the manifest was empty — the UI showed nothing.

## Tool access for subagents

Subagents don't reliably inherit MCP tools from the parent process. If your skill depends on MCP tools:

**Parent process uses:** MCP tools (navigate, screenshot, create resources)
**Subagents use:** HTTP API via `curl` (or whichever direct access method the backend provides)

The HTTP API is typically the same endpoint the MCP tool calls internally — `curl` cuts the intermediary and works 100% of the time.
...
- The tool access pattern is non-obvious (curl instead of Write, API instead of MCP)
...
| Race | Cause | Fix |
|------|-------|-----|
| Agent writes before manifest exists | Setup and agents launched simultaneously | Two-phase build |
