...
- **AUDIT = deterministic + reasoning** — the validator runs scriptable checks, then outputs an **LLM Reasoning Audit** reminder with rules that require judgment (naming, anemia, N+1, domain/infra leaks). The agent must act on both phases and present findings in a table.
- **Skill flag management** — when spawning a background sub-agent, the `skill-active` flag in `.claude/project-setup.json` is set to `false` during wait (prevents stop-hook loop) and `true` when REVIEW begins.
