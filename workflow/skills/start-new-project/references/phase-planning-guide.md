...
4. **TDD is mandatory — tests first, not alongside.** Write the test before the implementation in every step that introduces new behavior. This is not optional — it's the single most important quality rule. Use vertical slices: one test → one implementation → repeat. Never write all tests first then all implementations (horizontal slicing produces bad tests). See `references/tdd-methodology.md` for the full methodology including good/bad test examples, mocking rules, and the refactor cycle.
