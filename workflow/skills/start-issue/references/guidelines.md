...
Tests must never touch production data. When the issue involves database changes or file I/O, include a checkbox to configure the test environment. Read `references/development-guidelines.md` § 1 for env file separation, runtime safety guards, high ports, and full teardown requirements.
