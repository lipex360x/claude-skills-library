...
5. **Dev startup/teardown scripts are mandatory for multi-service projects.** Check if `scripts/dev-start.sh` and `scripts/dev-stop.sh` exist. When the project has infrastructure signals (docker-compose, multiple services, database) but these scripts are missing, the plan MUST include a Step to create them. The startup script orchestrates the full local environment in order: pre-flight checks → port cleanup → infrastructure (DB, containers) → migrations → seed → test user → services → health check → summary. The teardown script kills services and stops infrastructure. Both support flags (`--skip-seed`, `--keep-db`). After creation, add a `## Scripts` section to ARCHITECTURE.md referencing both scripts. This is stack-agnostic — adapt to whatever the project uses (Docker, Supabase CLI, local processes, virtualenvs, etc.).
