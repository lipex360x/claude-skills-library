...
- **Test isolation.** Tests must never touch production data. When the project uses a database, include a step to configure a separate test database (e.g., `:memory:` SQLite, dedicated test schema, Docker container, `.env.test` with `DATABASE_URL`). When the project uses a cloud service (Supabase, Firebase, PlanetScale, Neon, etc.), prefer running a **local instance via Docker** for tests — most cloud services offer local emulators (`supabase start`, `firebase emulators:start`) that replicate the cloud environment without network latency and with zero risk of polluting production data. When tests produce files, use a temporary directory that is cleaned up after each run. Include the test environment setup as an early checkbox in the first Part — before any test can run, the isolation boundary must exist. Tests that leak data into production are worse than no tests because they create false confidence.
