...
5. **Test isolation via docker-compose.** Orchestrate the test environment with `docker-compose.test.yml` (or a `test` profile in the main compose file) so any developer can run the full test stack with a single command. For cloud services (Supabase, Firebase, PlanetScale), include their local emulators as compose services. Point tests at local containers via `.env.test`. This guarantees portability — same setup on any machine and in CI, zero risk of polluting production data. Tests that leak into production are a liability, not an asset.
