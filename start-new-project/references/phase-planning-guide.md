...
5. **Test isolation via docker-compose.** Orchestrate the test environment with `docker-compose.test.yml` (or a `test` profile in the main compose file). For cloud services, include local emulators as compose services. Keep `.env.local` pointing at remote services; inject local URLs only in test context (`.env.test`, test runner config, compose env vars). Add a runtime safety guard (`global-setup.ts`) that verifies URLs point to `127.0.0.1`/`localhost` before running — abort if not. Tests that leak into production are a liability, not an asset.
